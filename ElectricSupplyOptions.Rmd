...
M_ISOCOMP_data %>% 
  ...
  scale_x_discrete(labels=month.abb) +
  theme_classic() +
    labs(x = "Month", y = "Annual Cost difference Fixed - 50/50 ($)", title = "Figure 8:" ~underline("Winning and looseing Months"))
...
Sensitivities Run at escalation all months at 30%

```{r Sensitivity Analysis, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}

#Make more analysis to 2014 the polar vortex year. 
R <- 1.3 # Esclation value to be multiplied to LBMP
...
Annual_Cost_Estimates_Run %>% 
  ggplot(aes(x = Strategy, y = Value)) +
  geom_col(aes(fill = Strategy)) +
  labs(x = "Strategy", y = "Annual Cost($)", title = "Senstitivy 30% Esclation" ~underline("Bar Chart of Annual Costs ($)") ) +
  theme_classic()
  
   # Work to make a table of the strtegies 
...
Senstitivy with a polor vortex.
