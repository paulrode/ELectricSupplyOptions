...
#  ggplot(aes(x = , y = LBMP_Avr)) +
#  geom_line() +
#  theme_classic() +
#  labs(x = "Date (Year - Month)", y = "Day Ahead Hourly LBMP ($/MWH)", title = "NYSISO LBMP Historical Trend by Month", caption = "ISO Market data from: https://www.nyiso.com/custom-reports" )
...
#LBMP Trend by Month
NYSISO_data %>% 
  mutate("YearMonth" = year(`TimeStamp`) + month(`TimeStamp`)/12) %>% 
  group_by(YearMonth) %>% 
