...
NYSISO_data %>% 
  ...
  scale_x_datetime() +
  theme_classic() +
  labs(x = "Date (Year)", y = "Day Ahead Hourly LBMP ($/MWH)", title = "NYSISO LBMP Historical Trend", caption = "ISO Market data from: https://www.nyiso.com/custom-reports" )
...
NYSISO_data_a %>%  
 ggplot(aes(x = ym , y = LBMP_Avr)) +
  geom_col() +
  theme_classic() +
  labs(x = "Date (Year - Month)", y = "Monthly Average Day Ahead Hourly LBMP ($/MWH)", title = "NYSISO LBMP Historical Trend by Month", caption = "ISO Market data from: https://www.nyiso.com/custom-reports" )
...
#LBMP Trend by Month
NYSISO_data %>% 
  mutate('Month' = month(TimeStamp)) -> NYSISO_data 
NYSISO_data$Month <- as.factor(NYSISO_data$Month)
NYSISO_data %>% 
...
ISOCOMP_data %>% 
  ...
  ISOCOMP_data %>% 
    ...
    summarise('Monthly_kWh' = sum(Energy), 'EnergyCost50_50' = sum(HourlyCost50_50), 'EnergyCostFixed' = sum(HourlyCostFixed)) %>% 
    mutate('EnergyUnit50_50' = EnergyCost50_50 / Monthly_kWh, 'EnergyUnitFixed' = EnergyCostFixed / Monthly_kWh) -> M_ISOCOMP_data
  
  M_ISOCOMP_data %>% 
    summarise('Cost50_50' = sum(EnergyCost50_50), 'CostFixed' = sum(EnergyCostFixed))
  
  M_ISOCOMP_data %>% 
      ggplot(aes(x = `month(TimeStamp)`, y = EnergyCostFixed)) +
  geom_line(color = "grey") +
  geom_line(aes(y = EnergyCost50_50), color = "green") +
  theme_classic()
