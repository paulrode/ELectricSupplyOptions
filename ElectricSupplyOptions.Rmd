...
NYSISO_data %>% 
  ...
  geom_boxplot( coef = 1.0) +
  scale_x_discrete(labels=month.abb) +
  labs(x = "Date (Month)", y = "Average LBMP ($/MWH)", title = "Figure 5:" ~underline("Box and Whisker Historical Average LBMP over BUILDING BUSINESS HOURS ($/MWH)") )
...
NYSISO_data %>% 
  ...
  scale_x_discrete(labels=month.abb) +
  theme_classic() +
labs(x = "Month", y = "Monthly Average Day Ahead Hourly LBMP ($/MWH)", title = "Figure 6:" ~underline("Historical Monthly Average LBMP by Month"))


# Table 7 Yearly averages 
NYSISO_data %>% 
  mutate('Year' =as.factor(year(TimeStamp))) %>% 
  ggplot(aes(x = Year, y = `DAM Zonal LBMP`)) +
  geom_boxplot( coef = 1.0) +
  scale_x_discrete() +
  labs(x = "Year", y = "Annual Average LBMP ($/MWH)", title = "Figure 7:" ~underline("Box and Whisker Average Annual LBMP ($/MWH)") )
